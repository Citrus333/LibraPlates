...
local defaultBlacklistNameColor = { 1.0, 0.28, 0.28, 1.0 };

local function NormalizeColor(value, fallback)
    local source = type(value) == 'table' and value or fallback;

    return {
        math.max(0, math.min(1, tonumber(source[1]) or fallback[1] or 1.0)),
        math.max(0, math.min(1, tonumber(source[2]) or fallback[2] or 1.0)),
        math.max(0, math.min(1, tonumber(source[3]) or fallback[3] or 1.0)),
        math.max(0, math.min(1, tonumber(source[4]) or fallback[4] or 1.0)),
    };
end
...
function playerBlacklist.GetDisplayName(player, fallback)
    ...
    if (settings.replaceName ~= false and playerBlacklist.IsListed(player) == true) then
        return 'Blacklisted';
    end
...
function playerBlacklist.GetDisplayNameColor(player)
    local settings = playerBlacklist.GetModelReplaceSettings();
    ...
    return NormalizeColor(settings.nameColor, defaultBlacklistNameColor);
end
...
function playerBlacklist.GetSignature(player)
    ...
    return table.concat({
        ...
        'replaceName=' .. tostring(settings.replaceName ~= false),
        'useFomor=' .. tostring(settings.modelReplaceUseFomor ~= false),
        'nameColor=' .. table.concat(settings.nameColor or {}, ','),
    }, ';');
end
...
function playerBlacklist.GetModelReplaceSettings()
    ...
    if (store.replaceName == nil) then
        store.replaceName = true;
    end
    store.nameColor = NormalizeColor(store.nameColor, defaultBlacklistNameColor);
