...
local magicPotPlatePositionOverride = {
    anchorBone = 12,
    worldOffsetY = 1.60,
};
local enemyPlatePositionOverrides = {
    ['air pot'] = magicPotPlatePositionOverride,
    ['ancient vessel'] = magicPotPlatePositionOverride,
    ['aura pot'] = magicPotPlatePositionOverride,
    ['clockwork pod'] = magicPotPlatePositionOverride,
    ['demonic millstone'] = magicPotPlatePositionOverride,
    ['droma'] = magicPotPlatePositionOverride,
    ['dustbuster'] = magicPotPlatePositionOverride,
    ['earth pot'] = magicPotPlatePositionOverride,
    ['eldhrimnir'] = magicPotPlatePositionOverride,
    ['fire pot'] = magicPotPlatePositionOverride,
    ['fired urn'] = magicPotPlatePositionOverride,
    ['hover tank'] = magicPotPlatePositionOverride,
    ['hovering hotpot'] = magicPotPlatePositionOverride,
    ['ice pot'] = magicPotPlatePositionOverride,
    ['illusory pot'] = magicPotPlatePositionOverride,
    ['jackpot'] = magicPotPlatePositionOverride,
    ['magic flagon'] = magicPotPlatePositionOverride,
    ['magic jar'] = magicPotPlatePositionOverride,
    ['magic jug'] = magicPotPlatePositionOverride,
    ['magic millstone'] = magicPotPlatePositionOverride,
    ['magic pot'] = magicPotPlatePositionOverride,
    ['magic urn'] = magicPotPlatePositionOverride,
    ['maledict millstone'] = magicPotPlatePositionOverride,
    ['maze lurker'] = magicPotPlatePositionOverride,
    ['mind hoarder'] = magicPotPlatePositionOverride,
    ['nightmare vase'] = magicPotPlatePositionOverride,
    ['olla grande'] = magicPotPlatePositionOverride,
    ['olla media'] = magicPotPlatePositionOverride,
    ['olla pequena'] = magicPotPlatePositionOverride,
    ['rogue receptacle'] = magicPotPlatePositionOverride,
    ['sacrificial goblet'] = magicPotPlatePositionOverride,
    ['sentient carafe'] = magicPotPlatePositionOverride,
    ['sinister seidel'] = magicPotPlatePositionOverride,
    ['sprinkler'] = magicPotPlatePositionOverride,
    ['steam cleaner'] = magicPotPlatePositionOverride,
    ['teekesselchen'] = magicPotPlatePositionOverride,
    ['thunder pot'] = magicPotPlatePositionOverride,
    ['water pot'] = magicPotPlatePositionOverride,
};
local user32 = nil;
...
local function ResolveEnemyAnchorBone(enemy)
    local enemyName = tostring(enemy ~= nil and enemy.name or ''):gsub('\170', ''):lower();
    local override = enemyPlatePositionOverrides[enemyName];

    if (override ~= nil and override.anchorBone ~= nil) then
        return tonumber(override.anchorBone) or enemyAnchorBone;
    end
...
local function ResolveEnemyWorldOffsetY(enemy)
    ...
    if (override ~= nil and override.worldOffsetY ~= nil) then
        return tonumber(override.worldOffsetY) or offsetY;
    end
...
local function ApplyEnemyPositionOverrides(enemy, worldMarker)
    ...
    local worldOffsetY = tonumber(override.worldOffsetY);

    if (worldOffsetY ~= nil) then
        worldMarker.plateWorldOffsetY = worldOffsetY;
        worldMarker.nameWorldOffsetY = worldOffsetY;
        worldMarker.hpBarWorldOffsetY = worldOffsetY;
    end

    if (override.anchorBone ~= nil) then
        worldMarker.anchorBone = tonumber(override.anchorBone) or worldMarker.anchorBone;
    end
...
local function AddEnemyMobInfoWidgetIcons(plateData, iconNames, iconSettings, defaultSettings, iconStyle, kind)
    if (plateData == nil or iconSettings == nil or iconSettings.enabled ~= true or iconNames == nil or #iconNames == 0) then
        return;
    end
...
local function GetEnemyMobInfoIconWidgetSettings(layoutStateName)
    return {
        behavior = state.GetWidgetSettings('Enemy', layoutStateName, 'Behavior icon', enemyBehaviorIconDefaults),
        detects = state.GetWidgetSettings('Enemy', layoutStateName, 'Detects icon', enemyDetectsIconDefaults),
        links = state.GetWidgetSettings('Enemy', layoutStateName, 'Links icon', enemyLinksIconDefaults),
    };
end

local function GetEnemyMobInfoIconWidgetSignature(widgetSettings)
...
local function IsEnemyMobInfoWidgetEnabled(settings)
    return settings ~= nil and settings.enabled == true;
end

local function UsesEnemyInfoAnchor(settings)
    local anchorTo = tostring(settings ~= nil and settings.anchorTo or 'Plate');
    return anchorTo == 'Behavior icon' or anchorTo == 'Detects icon' or anchorTo == 'Links icon' or anchorTo == 'Job' or anchorTo == 'Level';
end

local function ShouldProcessEnemyMobInfoWidgets(widgetSettings)
    widgetSettings = widgetSettings or {};

    return
        IsEnemyMobInfoWidgetEnabled(widgetSettings.behavior) or
        IsEnemyMobInfoWidgetEnabled(widgetSettings.detects) or
        IsEnemyMobInfoWidgetEnabled(widgetSettings.links);
end

local function EstimateTextFallbackSize(text, fontSize, minWidth)
    local size = math.max(8, tonumber(fontSize) or 12);
    local width = math.max(tonumber(minWidth) or size, (string.len(tostring(text or '')) * size * 0.58) + 4);

    return width, size + 4;
end

local function BuildMissingEnemyInfoAnchorRects(context)
    context = context or {};
    local widgetSettings = context.mobInfoIconWidgetSettings or {};
    local jobSettings = context.jobSettings or {};
    local levelSettings = context.levelSettings or {};
    local jobWidth, jobHeight = EstimateTextFallbackSize(context.jobText, jobSettings.textSize or jobDefaults.textSize, 24);
    local levelWidth, levelHeight = EstimateTextFallbackSize(context.levelText, levelSettings.textSize or levelDefaults.textSize, 34);
    local fallbacks = {};
    local definitions = {
        {
            kind = 'job',
        ...
        {
            kind = 'Behavior icon',
            settings = widgetSettings.behavior,
            defaults = enemyBehaviorIconDefaults,
        },
        {
            kind = 'Detects icon',
            settings = widgetSettings.detects,
            defaults = enemyDetectsIconDefaults,
        },
        {
            kind = 'Links icon',
            settings = widgetSettings.links,
        ...
        },
        {
            ...
            enabled = levelSettings.enabled == true,
            width = levelWidth,
            height = levelHeight,
            defaultY = -54,
        },
    ...
    local anyAnchored =
        UsesEnemyInfoAnchor(jobSettings) or
        UsesEnemyInfoAnchor(levelSettings) or
        UsesEnemyInfoAnchor(widgetSettings.behavior) or
        UsesEnemyInfoAnchor(widgetSettings.detects) or
        UsesEnemyInfoAnchor(widgetSettings.links);
    ...
    for _, definition in ipairs(definitions) do
        ...
        if (definition.enabled == true or IsEnemyMobInfoWidgetEnabled(settings) == true or anyAnchored == true) then
            ...
            local offsetY = tonumber(settings.offsetY) or tonumber(defaults.offsetY) or tonumber(definition.defaultY) or 0;

            fallbacks[#fallbacks + 1] = {
                kind = definition.kind,
                x = 512 - (width * 0.5) + offsetX,
                y = 256 - (height * 0.5) + offsetY,
                w = width,
                h = height,
                padding = 4,
                layout = {
    ...
    return (#fallbacks > 0) and fallbacks or nil;
end
...
local function AddEnemyMobInfoIconWidgetsToPlate(plateData, layoutStateName, mobInfo, globalSettings, widgetSettings)
    ...
    for _, iconName in ipairs(mobInfoData.GetFlags(mobInfo)) do
        if (iconName == 'AggroHQ' or iconName == 'AggroNQ' or iconName == 'PassiveHQ' or iconName == 'PassiveNQ') then
            behaviorIcons[#behaviorIcons + 1] = iconName;
        elseif (iconName == 'Link') then
            linkIcons[#linkIcons + 1] = iconName;
        else
...
local function GetAggroText(iconNames)
    if (iconNames == nil or #iconNames == 0) then
        return nil;
    end

    for _, iconName in ipairs(iconNames) do
        if (iconName == 'AggroHQ' or iconName == 'AggroNQ') then
            return 'Aggro';
        elseif (iconName == 'PassiveHQ' or iconName == 'PassiveNQ') then
            return 'Passive';
        end
    end
...
local function AddPeerAggroText(texts, iconNames, peerSettings, globalSettings)
    local text = GetAggroText(iconNames);
    ...
    if (options.color == nil) then
        options.color = { 1.0, 1.0, 1.0, 1.0 };
    end

    AddMobInfoText(texts, text, x, y, globalSettings, options);
end
...
local function NormalizeDifficultyKey(value)
    local key = string.lower(tostring(value or '')):gsub('[%s%p]+', '');

    if (key == 'tw' or key == 'tooweak' or key == 'liketooweak') then return 'tw'; end
    if (key == 'ep' or key == 'easyprey' or key == 'likeeasyprey') then return 'ep'; end
    if (key == 'dc' or key == 'decentchallenge' or key == 'likedecentchallenge') then return 'dc'; end
    if (key == 'em' or key == 'evenmatch' or key == 'likeevenmatch') then return 'em'; end
    if (key == 't' or key == 'tough' or key == 'liketough') then return 't'; end
    if (key == 'vt' or key == 'verytough' or key == 'likeverytough') then return 'vt'; end
    if (key == 'it' or key == 'incrediblytough' or key == 'likeincrediblytough') then return 'it'; end

    return nil;
end

local function GetPeerDifficultyKey(mobInfo)
    local infoKey = NormalizeDifficultyKey(mobInfo ~= nil and mobInfo.Difficulty or nil);
...
local function BuildMobInfoRows(enemy, globalSettings, peerSettings)
    ...
    if (peerSettings.showAggro ~= false) then
        ...
        AddPeerAggroText(texts, aggroIcons, peerSettings, globalSettings);
    end
...
local function GetDifficultyKey(viewerLevel, mobLevel)
    ...
    -- CatsEye / retail-style fallback brackets:
    -- TW: 8+ below, EP: 5-7 below, DC: 3-4 below,
    -- EM: 2 below to 2 above, T: 3-5 above, VT: 6-8 above, IT: 9+ above.
    if (delta <= -8) then return 'tw'; end
    if (delta <= -5) then return 'ep'; end
    if (delta <= -3) then return 'dc'; end
    if (delta <= 2) then return 'em'; end
    if (delta <= 5) then return 't'; end
    if (delta <= 8) then return 'vt'; end
    return 'it';
end
...
local function GetDifficultyColor(settings, defaults, mobInfo)
    ...
    local key = NormalizeDifficultyKey(mobInfo ~= nil and mobInfo.Difficulty or nil)
        or GetDifficultyKey(GetViewerLevel(), mobInfo ~= nil and (mobInfo.MaxLevel or mobInfo.Level or mobInfo.MinLevel));
...
local function GetDifficultyOutlineColor(settings, defaults, mobInfo)
    if (settings == nil or settings.difficultyColorsEnabled ~= true) then
    ...
    if (key == 'tw') then return settings.twOutlineColor or defaults.twOutlineColor or settings.outlineColor or defaults.outlineColor; end
    if (key == 'ep') then return settings.epOutlineColor or defaults.epOutlineColor or settings.outlineColor or defaults.outlineColor; end
    if (key == 'dc') then return settings.dcOutlineColor or defaults.dcOutlineColor or settings.outlineColor or defaults.outlineColor; end
    if (key == 'em') then return settings.emOutlineColor or defaults.emOutlineColor or settings.outlineColor or defaults.outlineColor; end
    if (key == 't') then return settings.tOutlineColor or defaults.tOutlineColor or settings.outlineColor or defaults.outlineColor; end
    if (key == 'vt') then return settings.vtOutlineColor or defaults.vtOutlineColor or settings.outlineColor or defaults.outlineColor; end
    if (key == 'it') then return settings.itOutlineColor or defaults.itOutlineColor or settings.outlineColor or defaults.outlineColor; end

    return settings.outlineColor or defaults.outlineColor;
end
...
local function AddLevelToPlate(plateData, levelText, levelSettings, mobInfo, globalSettings)
    ...
    plateData.badges[#plateData.badges + 1] = {
        ...
        textOutlineColor = GetDifficultyOutlineColor(levelSettings, levelDefaults, mobInfo),
...
local function QueueCachedEnemy(enemy, cached, stateName, importantAlwaysOnTop, hpPercent, liveHpBarStyle)
    ...
    local worldMarker = ApplyEnemyPositionOverrides(enemy, targeting.ApplyPlateScalingSettings({
        hpBar = liveHpBarStyle or { enabled = false },
...
local function QueueFreshIdleCache(enemy)
    ...
    if (cached.dynamicHpBar == true) then
        return false;
    end

    local liveHpBarStyle = cached.hpBar;
    if (liveHpBarStyle ~= nil and liveHpBarStyle.enabled == true and tostring(liveHpBarStyle.text or '') ~= '') then
        local copy = {};
        for key, value in pairs(liveHpBarStyle) do
            copy[key] = value;
        end
        copy.text = tostring(math.floor(ClampPercent(enemy.hpPercent, 100) + 0.5)) .. '%';
        liveHpBarStyle = copy;
    end

    if (QueueCachedEnemy(enemy, cached, 'Idle', false, ClampPercent(enemy.hpPercent, 100), liveHpBarStyle) == true) then
...
local function BuildEnemyCacheSignature(context, useLiveHpBar)
    ...
    return table.concat({
        ...
        'hp=' .. (context.hpBarSettings.enabled == true and useLiveHpBar ~= true and tostring(math.floor((tonumber(context.hpPercent) or 0) + 0.5)) or ''),
        ...
        'bg=' .. SettingKey(context.backgroundSettings, { 'enabled', 'width', 'height', 'offsetX', 'offsetY', 'texture', 'color', 'borderColor', 'borderSize', 'anchorTo', 'anchorPoint', 'anchorCollapse', 'anchorSpacing' }),
...
local function BuildEnemyPlateData(context)
    ...
    local plateData = {
        ...
        cast = context.castPercent,
        targetMarker = context.targetMarker,
        background = {
            enabled = context.backgroundSettings.enabled == true,
        ...
        anchorMap = {
            ['Background'] = 'background', ['Name'] = 'name', ['HP Bar'] = 'hp', ['MP Bar'] = 'mp', ['TP Bar'] = 'tp', ['Cast bar'] = 'cast',
            ['Job'] = 'job', ['Level'] = 'level', ['ID'] = 'id', ['Game mode icon'] = 'gameModeIcon', ['Linkshell icon'] = 'linkshellIcon',
            ['Behavior icon'] = 'Behavior icon', ['Detects icon'] = 'Detects icon', ['Links icon'] = 'Links icon', ['Bazaar icon'] = 'bazaarIcon',
        ...
        hpBar = {
            enabled = context.hpBarSettings.enabled == true,
            ...
            text = (context.hpBarSettings.showPercent == true) and (tostring(math.floor(context.hpPercent + 0.5)) .. '%') or '',
...
local function CanUseLiveHpBar(settings)
    settings = settings or {};
    ...
    if (settings.showValue == true) then
        return false;
    end
...
local function MakeLiveHpBarStyle(settings, color, hpPercent, globalSettings)
    settings = settings or {};

    if (CanUseLiveHpBar(settings) ~= true) then
        return { enabled = false };
    end

    return {
        enabled = true,
        worldWidth = math.max(0.04, (tonumber(settings.width) or 180) / 430),
        worldHeight = math.max(0.006, (tonumber(settings.height) or 12) / 260),
        worldOffsetX = (tonumber(settings.offsetX) or 0) / 430,
        worldOffsetY = (tonumber(settings.offsetY) or 0) / 260,
        color = color,
        backgroundColor = settings.backgroundColor,
        borderColor = settings.borderColor,
        borderWorldSize = math.max(0, (tonumber(settings.borderSize) or 0) / 260),
        text = (settings.showPercent == true) and (tostring(math.floor((tonumber(hpPercent) or 0) + 0.5)) .. '%') or '',
        textWorldOffsetX = (tonumber(settings.textOffsetX) or 0) / 430,
        textWorldOffsetY = (tonumber(settings.textOffsetY) or 0) / 260,
        fontFamily = fonts.GetRole(globalSettings or {}, settings.useSmallFont == true),
        fontSize = textScale.ToTextureFontSize(settings.fontSize, barDefaults.fontSize),
        ...
        textOutlineSize = tonumber(settings.textOutlineSize) or 0,
    };
end
...
local function ApplyEnemyStatusWidgets(context, plateData)
    ...
    plateData.anchorFallbackRects = BuildMissingEnemyInfoAnchorRects(context);
    AddEnemyMobInfoIconWidgetsToPlate(plateData, context.layoutStateName, context.mobInfo, context.globalSettings, context.mobInfoIconWidgetSettings);
end
...
local function QueueEnemy(enemy)
    ...
    if (
        ...
        ApplyPeerName(plateData, peerSettings, context.globalSettings);
        ApplyPeerHpBar(plateData, peerSettings, context.hpPercent, context.globalSettings);
    ...
    local worldMarker = ApplyEnemyPositionOverrides(enemy, targeting.ApplyPlateScalingSettings({
        hpBar = useLiveHpBar == true and liveHpBarStyle or { enabled = false },
