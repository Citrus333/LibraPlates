...
local function GetPlayerTargetIndex()
    ...
    return tonumber(targetIndex) or 0;
end
...
-- Superwarp sends both of these before choosing a Home Point destination.
-- 0x016 refreshes the player's position/state and 0x114 asks the server for
-- the Home Point map data.  Its state machine waits for the 0x05C responses
-- produced by the automated 0x05B menu choices.
local function BuildUpdateRequestPacket(targetIndex)
    return struct.pack(
        'bbbbHH',
        0x16,
        0x02,
...
local function BuildHomePointMapRequestPacket()
    return struct.pack(
        'bbbb',
        0x14,
        0x01,
...
local function QueueWarpSequence(menuId)
    ...
    if (unlockStatusReliable == true and unlockStatus[destinationIndex] == false) then
        log.Warn('Home Point warp stopped: destination was reported as locked.');
    ...
    -- Match Superwarp's proven Home Point handshake.  Sending the destination
    -- packets immediately after the menu opens can leave the client inside an
    -- event that never completes.
    QueueAction(0.00, function()
        if (pending == active) then
            -- Superwarp waits for the 0x05C generated by this menu change
            -- before it sends the map request and destination selection.
            active.stage = 'waiting-menu-response';
            active.menuId = menuId;
            active.startedAt = Now();

            SendOutgoingPacket(0x016, BuildUpdateRequestPacket(GetPlayerTargetIndex()), 'update request');
            SendOutgoingPacket(0x05B, BuildMenuPacket(targetId, targetIndex, zoneId, menuId, 8, 0, true), 'menu change');
        end
    end);
end

local function QueueDestinationSelection(active)
    local destination = active.destination or {};
    local destinationIndex = tonumber(destination.warpIndex);
    local menuId = tonumber(active.menuId) or tonumber(active.fallbackMenuId) or 8700;

    active.stage = 'preparing-destination';
    active.startedAt = Now();
    pending = active;

    QueueAction(0.00, function()
        if (pending == active) then
            SendOutgoingPacket(0x114, BuildHomePointMapRequestPacket(), 'request home point map');
        end
    end);

    QueueAction(0.20, function()
        if (pending == active) then
            -- This selection produces the second 0x05C.  Superwarp waits for
            -- that response before sending the non-automated confirmation.
            active.stage = 'waiting-selection-response';
            active.startedAt = Now();
            SendOutgoingPacket(0x05B, BuildMenuPacket(active.targetId, active.targetIndex, active.zoneId, menuId, 2, destinationIndex, true), 'select destination');
        end
    end);
end

local function ConfirmDestination(active)
    local destination = active.destination or {};
    local destinationIndex = tonumber(destination.warpIndex);
    local menuId = tonumber(active.menuId) or tonumber(active.fallbackMenuId) or 8700;

    SendOutgoingPacket(0x05B, BuildMenuPacket(active.targetId, active.targetIndex, active.zoneId, menuId, 2, destinationIndex, false), 'confirm destination');
    if (debugEnabled == true) then
...
function homePointWarp.HandlePacketIn(e)
    ...
    if (e.id == 0x05C) then
        if (pending.stage == 'waiting-menu-response') then
            QueueDestinationSelection(pending);
            return;
        elseif (pending.stage == 'waiting-selection-response') then
            local active = pending;
            pending = nil;
            ConfirmDestination(active);
            return;
        end
    end
    ...
    -- Superwarp blocks the native menu packet while its packet state machine
    -- owns the interaction.  This prevents the game UI and the automation from
    -- advancing the same event independently.
    e.blocked = true;
...
function homePointWarp.Update()
    if (pending ~= nil and (Now() - (tonumber(pending.startedAt) or 0)) > 4.0) then
        local active = pending;
        local menuId = tonumber(active.menuId) or tonumber(active.fallbackMenuId);
        log.Warn('Home Point warp timed out waiting for the server response. Sending a Home Point event cancellation.');
        pending = nil;
        queue = {};
        if (menuId ~= nil) then
            QueueCancelMenu(active, menuId, 'cancel timed out home point warp');
        end
        return;
    end
