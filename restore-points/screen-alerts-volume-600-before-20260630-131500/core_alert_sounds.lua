...
local function WriteU8(output, value)
    output[#output + 1] = string.char(math.max(0, math.min(255, value)));
end

local function WriteI16(output, value)
...
local function AmplifyPcmData(pcmData, bitsPerSample, gain)
    ...
    if (bitsPerSample == 16) then
        for index = 1, #pcmData - 1, 2 do
            local sample = ReadU16(pcmData, index);
            if (sample >= 32768) then
                sample = sample - 65536;
            end

            WriteI16(output, math.floor((sample * gain) + 0.5));
        end

        return table.concat(output);
    elseif (bitsPerSample == 8) then
        for index = 1, #pcmData do
            local sample = (pcmData:byte(index) or 128) - 128;
            WriteU8(output, math.floor((sample * gain) + 128.5));
        end
...
local function LoadDirectSoundBuffer(path, gain)
    ...
    local pcmData = AmplifyPcmData(data:sub(dataOffset, dataOffset + dataLength - 1), format.wBitsPerSample, gain);
...
local function PlayDirectSound(fileName, path, volume)
    local boost = volume > 100 and math.floor(volume + 0.5) or 100;
    local bufferKey = tostring(fileName) .. '#' .. tostring(boost);
    local buffer = soundState.buffers[bufferKey];
    if (buffer == false) then
        return false;
    end
    if (buffer == nil) then
        buffer = LoadDirectSoundBuffer(path, boost / 100);
    ...
    local ok = pcall(function()
        buffer.lpVtbl.SetVolume(buffer, VolumeToDb(math.min(100, volume)));
