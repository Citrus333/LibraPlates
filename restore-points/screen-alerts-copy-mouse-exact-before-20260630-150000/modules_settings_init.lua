...
local generalSections = T{ 'Profiles', 'Theme', 'Native UI', 'Mouse', 'Visibility', 'Blacklist', 'Screen Alerts', 'Scaling', 'Performance' };
...
function LibraPlatesSettingsDrawEnemyAlertsSection(useBoxedPage)
    ...
    local function DrawLane(label, prefix, enabledKey, showHeader)
        ...
        DrawCheckbox('Play sounds', settings[prefix .. 'SoundEnabled'] == true, function(value)
        ...
        if (imgui.Button ~= nil and imgui.Button('Preview sound##EnemyAlertsPlate' .. label .. 'SoundPreview') == true) then
            alertSounds.Play(soundFile, settings.soundVolume or 100);
    ...
    local function DrawGeneralControls(showHeader)
        ...
        imgui.TextWrapped('Screen Alerts shows short on-screen messages and can play sounds when events are triggered, such as enemy casts, readied abilities, VNM messages, or learned Blue Magic. Built-in alerts can be toggled here, and custom text triggers can be added.');
        imgui.Spacing();
        ...
        local soundVolume, soundVolumeChanged = DrawPlacementNumber('Sound volume', settings.soundVolume or 100, 0, 600, 25, 'EnemyAlertsPlateSoundVolume');
    ...
    local function DrawContent()
        ...
        DrawSectionDivider();
        DrawLane('Job abilities', 'ability', true);
    ...
    local function DrawAlertsPanel(label, render)
        local panelGap = 8;
        local panelPadX = 16;
        local panelPadY = 10;
        local topGap = (label == 'Screen Alerts') and 2 or panelGap;
        ...
        if (
            drawList ~= nil and
            drawList.ChannelsSplit ~= nil and
            drawList.ChannelsSetCurrent ~= nil and
            drawList.ChannelsMerge ~= nil and
            imgui.GetColorU32 ~= nil and
            imgui.SetCursorScreenPos ~= nil
        ) then
            local availWidth = select(1, GetContentRegionAvail());
            local cardWidth = math.max(260, (tonumber(availWidth) or 260) - 16);
            local startX, startY = GetCursorScreenPos();

            drawList:ChannelsSplit(2);
            drawList:ChannelsSetCurrent(1);
            imgui.SetCursorScreenPos({ startX + panelPadX, startY + panelPadY });
            DrawYellowHeader(tostring(label or ''));
            imgui.Spacing();
            render();
            local endX, endY = GetCursorScreenPos();
            local panelHeight = math.max(32, (tonumber(endY) or startY) - startY + panelPadY);

            drawList:ChannelsSetCurrent(0);
            drawList:AddRectFilled(
                { startX, startY },
                { startX + cardWidth, startY + panelHeight },
                imgui.GetColorU32(LibraPlatesSettingsPalette.panelBg),
                0
            );
            drawList:ChannelsMerge();

            imgui.SetCursorScreenPos({ startX, startY + panelHeight });
            return;
        end
        ...
        if (imgui.BeginTable ~= nil and imgui.TableSetupColumn ~= nil) then
            ...
            if (imgui.BeginTable('##ScreenAlertsPanel' .. tostring(label or ''), 1, (_G.ImGuiTableFlags_RowBg or 0), { cardWidth, 0 })) then
                imgui.TableSetupColumn('##card', 0, cardWidth);
                imgui.TableNextRow();
                if (imgui.TableSetBgColor ~= nil and imgui.GetColorU32 ~= nil) then
                    imgui.TableSetBgColor(_G.ImGuiTableBgTarget_RowBg0 or 1, imgui.GetColorU32(LibraPlatesSettingsPalette.panelBg));
                    imgui.TableSetBgColor(_G.ImGuiTableBgTarget_RowBg1 or 2, imgui.GetColorU32(LibraPlatesSettingsPalette.panelBg));
                end
                imgui.TableNextColumn();
    ...
    local function DrawAlertsBreadcrumb()
        imgui.TextColored({ 1.0, 1.0, 1.0, 1.0 }, 'Selected: ');
        if (imgui.SameLine ~= nil) then imgui.SameLine(); end
        imgui.TextColored({ 1.0, 1.0, 1.0, 1.0 }, 'Settings');
        if (imgui.SameLine ~= nil) then imgui.SameLine(); end
        imgui.TextColored({ 1.0, 1.0, 1.0, 1.0 }, '>');
        if (imgui.SameLine ~= nil) then imgui.SameLine(); end
        imgui.TextColored({ 1.0, 0.84, 0.0, 1.0 }, 'Screen Alerts');
    end

    if (useBoxedPage == true) then
        if (imgui.GetWindowDrawList ~= nil and imgui.GetColorU32 ~= nil) then
            local bgX, bgY = GetCursorScreenPos();
            local bgWidth, bgHeight = GetContentRegionAvail();
            local drawList = imgui.GetWindowDrawList();
            drawList:AddRectFilled(
                { bgX, bgY },
                { bgX + math.max(1, tonumber(bgWidth) or 1), bgY + math.max(1, tonumber(bgHeight) or 1) },
                imgui.GetColorU32(LibraPlatesSettingsPalette.shellBg),
                0
            );
        end

        DrawAlertsBreadcrumb();
        ...
        DrawAlertsPanel('Job abilities', function()
            DrawLane('Job abilities', 'ability', nil, false);
        end);
    else
        DrawContent();
    end
