...
local function SendOutgoingPacket(id, packedData, description)
    ...
    if (ok ~= true) then
        log.Warn('Campaign teleport packet failed ' .. tostring(description or '') .. ' id=0x' .. string.format('%03X', tonumber(id) or 0) .. ' err=' .. tostring(err));
        return false;
    end

    if (debugEnabled == true) then
        log.Info('Campaign teleport sent packet ' .. tostring(description or '') .. ' id=0x' .. string.format('%03X', tonumber(id) or 0) .. ' bytes=' .. FormatBytes(packedData, 32));
    end
...
local function FindEventId(data)
    ...
    for offset = 4, math.max(4, #data - 1) do
        ...
        if (campaignData.eventIds ~= nil and value ~= nil) then
            for _, eventId in pairs(campaignData.eventIds) do
...
local function QueueTeleportSequence(eventId)
    ...
    local selection = tonumber(destination.index);

    if (selection == nil) then
        log.Warn('Campaign teleport missing destination index for ' .. tostring(destination.name or ''));
        pending = nil;
        return;
    end

    QueueAction(0.20, function()
        SendOutgoingPacket(0x05B, BuildMenuPacket(active.targetId, active.targetIndex, active.zoneId, eventId, 50, 0, true), 'enter teleport list');
    end);

    QueueAction(1.10, function()
        SendOutgoingPacket(0x05B, BuildMenuPacket(active.targetId, active.targetIndex, active.zoneId, eventId, selection, 0, true), 'select destination');
    end);

    QueueAction(2.00, function()
        SendOutgoingPacket(0x05B, BuildMenuPacket(active.targetId, active.targetIndex, active.zoneId, eventId, selection, 0, false), 'confirm destination');
        if (debugEnabled == true) then
            log.Info('Campaign teleport sent: ' .. tostring(destination.name or '') .. ' index=' .. tostring(selection));
        end
    end);
...
function campaignTeleport.Request(destination, context)
    context = context or {};
    ...
    local eventId = campaignData.eventIds[zoneId];

    if (targetIndex <= 0 or targetId <= 0) then
        log.Warn('Campaign teleport failed: no valid Campaign Arbiter target.');
        return false;
    end

    if (eventId == nil) then
        log.Warn('Campaign teleport failed: this Campaign Arbiter event has not been captured yet.');
        return false;
    end
    ...
    if (SendOutgoingPacket(0x01A, BuildNpcPokePacket(targetId, targetIndex), 'poke campaign arbiter') ~= true) then
        pending = nil;
        return false;
    end
    ...
    if (debugEnabled == true) then
        log.Info('Campaign teleport queued: ' .. tostring(destination.name or '') .. ' using captured event=' .. tostring(eventId) .. '.');
    end

    return true;
end

function campaignTeleport.HandlePacketIn(e)
    ...
    if (debugEnabled == true and (e.id == 0x032 or e.id == 0x034 or e.id == 0x05C or e.id == 0x052)) then
        watchUntil = Now() + 12;
        log.Info('Campaign teleport saw incoming id=0x' .. string.format('%03X', tonumber(e.id) or 0) .. ' size=' .. tostring(e.size or #e.data) .. ' bytes=' .. FormatPacketString(e.data_modified or e.data, 48));
    end
...
function campaignTeleport.HandlePacketOut(e)
    ...
    if (pending ~= nil or Now() < (tonumber(watchUntil) or 0) or e.id == 0x01A) then
        if (e.id == 0x01A or e.id == 0x05B or e.id == 0x05C or e.id == 0x114 or e.id == 0x016) then
            log.Info('Campaign teleport saw outgoing id=0x' .. string.format('%03X', tonumber(e.id) or 0) .. ' size=' .. tostring(e.size or #e.data) .. ' bytes=' .. FormatPacketString(e.data_modified or e.data, 48));
        end
    end
end

function campaignTeleport.Update()
    if (pending ~= nil and (Now() - (tonumber(pending.startedAt) or 0)) > 4.0) then
        log.Warn('Campaign teleport timed out waiting for menu.');
        pending = nil;
        queue = {};
        return;
    end
...
function campaignTeleport.SetDebugEnabled(value)
    debugEnabled = value == true;
end

return campaignTeleport;
