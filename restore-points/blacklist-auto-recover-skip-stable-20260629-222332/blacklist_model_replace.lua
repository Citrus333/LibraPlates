...
function blacklistModelReplace.HandlePacketIn(e)
    ...
    if (entry ~= nil and settings.modelReplaceUseFomor ~= false and ShouldSuppressBlacklistRecoveryWrite(player) ~= true) then
        WriteCostumeId(ffiRef, packet, fixedModelId);
        QueueBlacklistModelRecovery(player, fixedModelId, fomorModels[raceFamily] ~= nil and fomorModels[raceFamily][1] or settings.modelReplaceHair);
