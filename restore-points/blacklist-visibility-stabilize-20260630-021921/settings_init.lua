...
function LibraPlatesSettingsDrawGeneralBlacklistSection()
    ...
    LibraPlatesSettingsDrawBoxedPage('BlacklistPageContent', function()
        LibraPlatesSettingsDrawBoxedPanel('Visual blacklist', function()
            DrawCheckbox('Rename players to Blacklisted', blacklistSettings.replaceName ~= false, function(value)
                blacklistSettings.replaceName = value == true;
                state.Save();
            end);
            uiTooltip.Info('When off, blacklisted players keep their normal character name on LibraPlates.');
            ...
            DrawCheckbox('Use Fomor appearance', blacklistSettings.modelReplaceUseFomor ~= false, function(value)
                ...
                if (value ~= true) then
                    blacklistModelReplace.QueueClearAll(30.0);
                end
                state.Save();
            end);
            uiTooltip.Info('When off, blacklisted players keep their normal body model.');

            local nextColor, colorChanged = DrawSettingsColor('Name color', blacklistSettings.nameColor, 'BlacklistNameColor');
            blacklistSettings.nameColor = nextColor;
