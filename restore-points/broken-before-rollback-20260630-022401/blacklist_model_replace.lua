...
local packetFomorReplacementEnabled = false;
...
local function HandlePlayerModelPacket00E(e, settings)
    ...
    if (packetFomorReplacementEnabled ~= true or settings.modelReplaceUseFomor == false) then
        return;
    end
...
function blacklistModelReplace.HandlePacketIn(e)
    ...
    if ((packetFomorReplacementEnabled ~= true or settings.modelReplaceUseFomor == false) and HasPendingClear() ~= true) then
        return;
    end
    ...
    if (
        entry ~= nil and
        packetFomorReplacementEnabled == true and
...
function blacklistModelReplace.QueueClearAll(seconds)
    pendingClearAllUntil = os.clock() + (tonumber(seconds) or 30.0);
end
...
function blacklistModelReplace.SetReplacementRace(value)
    ...
    return true, resolvedFamily, GetFixedFomorModelId(settings, GetRaceModelKey(number), resolvedFamily);
end
...
function blacklistModelReplace.GetDebugStatusText()
    ...
    return 'Blacklist model replacement debug=' .. tostring(debugEnabled == true) ..
        ...
        ' packetFomorRuntime=' .. tostring(packetFomorReplacementEnabled == true) ..
