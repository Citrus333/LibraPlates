...
local function QueueBlacklistModelRecovery(player, fixedModelId, originalHair)
    if (player == nil) then
        return;
    end
...
function blacklistModelReplace.HandlePacketIn(e)
    ...
    local stableRecovered = IsStableBlacklistRecoveredActor(player);
    ...
    if (
        ...
        QueueBlacklistModelRecovery(player, fixedModelId, packet[0x48]);
