return {
    ["npcs"] = {
        ["Abbudin"] = {
            ["_source"] = "npc",
            ["icon"] = "Dialogue.png",
            ["type"] = "Citizen",
        },
        ["Aileen"] = {
            ["_source"] = "npc",
            ["icon"] = "QuestAssociate.png",
            ["note"] = "Involved in Quest:\
* Too Many Chefs\
",
            ["type"] = "Quest Associate",
        },
        ["Alois"] = {
            ["_source"] = "npc",
            ["icon"] = "QuestAssociate.png",
            ["note"] = "Involved in Quests:\
* Faded Promises\
Involved in Missions:\
* Bastok Mission 2-2: Wading Beasts\
* Bastok Mission 9-1: The Salt of the Earth",
            ["type"] = "Quest Associate",
        },
        ["Amulya"] = {
            ["_source"] = "npc",
            ["icon"] = "GuildMerchant.png",
            ["note"] = "Notes:\
- Open from 8:00 to 23:00.\
- Has independent stock from Vicious Eye.",
            ["type"] = "Smithing Merchant",
        },
        ["Ayame"] = {
            ["_source"] = "npc",
            ["icon"] = "QuestGiver.png",
            ["note"] = "Starts Quests:\
* A Question of Faith\
* Out of the Depths\
* Return to the Depths\
* True Strength (MNK AF3)\
Involved in Quests:\
* Faded Promises\
* Lure of the Wildcat (Bastok)\
* Trust: Bastok\
Involved in Missions:\
* Bastok Mission 2-1: The Crystal Line\
* Bastok Mission 3-1: The Four Musketeers\
* Bastok Mission 6-2: The Pirates' Cove\
* Bastok Mission 9-2: Where Two Paths Converge\
* Promathia Mission 1-3: The Mothercrystals\
* Promathia Mission 3-1: Call of the Wyrmking\
* Aht Urhgan Mission 38: Stirrings of War\
* A Crystalline Prophecy (Mission)\
",
            ["type"] = "Mythril Musketeer",
        },
        ["Baldric"] = {
            ["_source"] = "npc",
            ["icon"] = "QuestGiver.png",
            ["note"] = "Starts Quests:\
* Stardust",
            ["type"] = "Quest Giver",
        },
        ["Brave Fang"] = {
            ["_source"] = "catseye_npc",
            ["icon"] = "QuestAssociate.png",
            ["link"] = "https://www.bg-wiki.com/ffxi/CatsEyeXI_Systems/Quests#X_Marks_the_Spot",
            ["note"] = "Involved in Quest:\
- X Marks the Spot\
- Exchanges the Onion Sword for the final augmented Degen.",
            ["type"] = "Corsair Quest Associate",
        },
        ["Bumimi"] = {
            ["_source"] = "npc",
            ["icon"] = "QuestNPC.png",
            ["note"] = "Involved in Quests:\
* Bait and Switch\
",
            ["type"] = "Quest NPC",
        },
        ["Celebratory Chest"] = {
            ["_source"] = "npc",
            ["icon"] = "SpecialMerchant.png",
            ["type"] = "Standard Merchant",
        },
        ["Cerberus"] = {
            ["_source"] = "npc",
            ["icon"] = "Dialogue.png",
            ["type"] = "Citizen",
        },
        ["Cid"] = {
            ["_source"] = "npc",
            ["icon"] = "QuestGiver.png",
            ["note"] = "Starts Quests:\
* Cid's Secret\
* Dark Puppet\
* Shoot First, Ask Questions Later\
Involved in Quests:\
* Chips\
* Hyper Active\
* Vegetable Vegetable Revolution\
* Vegetable Vegetable Crisis\
* Vegetable Vegetable Frustration\
Involved in Missions:\
* Bastok Mission 1-2: A Geological Survey\
* Bastok Mission 2-1: The Crystal Line\
* Bastok Mission 3-2: To the Forsaken Mines\
* Bastok Mission 5-1: Darkness Rising\
* Bastok Mission 5-2: Xarcabard, Land of Truths\
* Bastok Mission 7-1: The Final Image\
* Bastok Mission 9-2: Where Two Paths Converge\
* Promathia Mission 1-3: The Mothercrystals\
* Promathia Mission 3-1: Call of the Wyrmking\
* Promathia Mission 3-4: Tending Aged Wounds\
* Promathia Mission 5-1: The Enduring Tumult of War\
* Promathia Mission 5-2: Desires of Emptiness\
* Promathia Mission 5-3: Three Paths\
* The Pursuit of Paradise (Tenzen's Path)\
* Where Messengers Gather (Ulmia's Path)\
* Past Sins (Louverance's Path)\
* Promathia Mission 6-3: More Questions Than Answers\
* Promathia Mission 6-4: One to be Feared\
* Promathia Mission 7-3: Fire in the Eyes of Men\
* Promathia Mission 7-4: Calm Before the Storm\
* Promathia Mission 8-4: Dawn\
* A Crystalline Prophecy (Mission)\
* The Voracious Resurgence #2: The Brygid Cup\
",
            ["type"] = "Engineer",
        },
        ["Cloud Walker"] = {
            ["_source"] = "npc",
            ["icon"] = "MarriageCertificate.png",
            ["note"] = "Wedding Support:\
- Bastok wedding registrar.\
- Issues Bastokan Marriage Certificates to eligible married couples.",
            ["type"] = "Wedding Registrar",
        },
        ["Crystal Crunch"] = {
            ["_source"] = "catseye_npc",
            ["icon"] = "CrystalCrunch.png",
            ["note"] = "CatsEyeXI:\
* Located near guilds.\
* Exchanges crystals and clusters\
at a 3-to-1 rate.",
            ["type"] = "Crystal Exchange",
        },
        ["Curator Moogle"] = {
            ["_source"] = "npc",
            ["icon"] = "SpecialEventCoordinator.png",
            ["type"] = "Special Event",
        },
        ["Dannion"] = {
            ["_source"] = "npc",
            ["icon"] = "Cutscene.png",
            ["note"] = "Notes:\
- Appears in cutscenes during Bait and Switch.\
- His dialogue changes as the temple switches are activated.",
            ["type"] = "Cutscene NPC",
        },
        ["Darha"] = {
            ["_source"] = "npc",
            ["icon"] = "QuestAssociate.png",
            ["note"] = "Involved in Quests:\
* Bait and Switch",
            ["type"] = "Quest Associate",
        },
        ["Dark Clouds"] = {
            ["_source"] = "npc",
            ["icon"] = "Dialogue.png",
            ["type"] = "Citizen",
        },
        ["Dietrich"] = {
            ["_source"] = "npc",
            ["icon"] = "Defender.png",
            ["type"] = "Defender",
        },
        ["Elayne"] = {
            ["_source"] = "npc",
            ["icon"] = "QuestAssociate.png",
            ["note"] = "Involved in Quests:\
* Stamp Hunt",
            ["type"] = "Quest Associate",
        },
        ["Elrica"] = {
            ["_source"] = "npc",
            ["icon"] = "QuestAssociate.png",
            ["note"] = "Involved in Quest:\
* Seeds of Flame\
* Waking the Colossus\
",
            ["type"] = "Quest Associate",
        },
        ["Esvin"] = {
            ["_source"] = "npc",
            ["icon"] = "QuestAssociate.png",
            ["note"] = "Involved in Quests:\
* Escutcheons",
            ["type"] = "Quest Associate",
        },
        ["Fariel"] = {
            ["_source"] = "npc",
            ["icon"] = "Defender.png",
            ["type"] = "Defender",
        },
        ["Ferghus"] = {
            ["_source"] = "npc",
            ["icon"] = "QuestGiver.png",
            ["note"] = "Starts Quests:\
* Too Many Chefs\
Involved in Quests:\
* Bait and Switch",
            ["type"] = "Quest Giver",
        },
        ["Ferredec"] = {
            ["_source"] = "catseye_npc",
            ["icon"] = "QuestNPC.png",
            ["note"] = "Crystal Warrior:\
* Starts quest: Wanting Ore.\
* Level 20 early gear quest.",
            ["type"] = "CW Early Gear",
        },
        ["Folzen"] = {
            ["_source"] = "npc",
            ["icon"] = "QuestAssociate.png",
            ["note"] = "Involved in Quests:\
* Bait and Switch",
            ["type"] = "Quest Associate",
        },
        ["Franziska"] = {
            ["_source"] = "npc",
            ["icon"] = "QuestNPC.png",
            ["note"] = "Involved in Quests:\
* Bastok Mission 7-2: On My Way\
",
            ["type"] = "Quest NPC",
        },
        ["Galina"] = {
            ["_source"] = "npc",
            ["icon"] = "Dialogue.png",
            ["type"] = "Citizen",
        },
        ["Gentle Tiger"] = {
            ["_source"] = "npc",
            ["icon"] = "Cutscene.png",
            ["note"] = "Involved in Quests:\
* Bait and Switch",
            ["type"] = "Cutscene NPC",
        },
        ["Ghemp"] = {
            ["_source"] = "npc",
            ["icon"] = "bsm_GuildMaster.png",
            ["note"] = "Notes:\
- Enrolls players in the Smithing Guild.\
- Accepts rank-up test items.",
            ["type"] = "Guild Master",
        },
        ["Givorj"] = {
            ["_source"] = "npc",
            ["icon"] = "PastEventWatcher.png",
            ["note"] = "Notes:\
* Replays cutscenes for this area.",
            ["type"] = "Past Event Watcher",
        },
        ["Glarociquet, T.K."] = {
            ["_source"] = "npc",
            ["icon"] = "ConquestOverseer.png",
            ["note"] = "Notes:\
- Serves San d'Orian citizens.\
- Casts Signet and recharges Conquest rings.\
- Accepts crystals and sells Conquest Point rewards.\
- Explains the Conquest system.",
            ["type"] = "Conquest Overseer",
        },
        ["Grohm"] = {
            ["_source"] = "npc",
            ["icon"] = "MissionAssociate.png",
            ["note"] = "Involved in Missions:\
* San d'Oria Mission 2-3: Journey Abroad\
* Windurst Mission 2-3: The Three Kingdoms",
            ["type"] = "Mission Associate",
        },
        ["Hadelind"] = {
            ["_source"] = "npc",
            ["icon"] = "Dialogue.png",
            ["type"] = "Citizen",
        },
        ["Hans"] = {
            ["_source"] = "npc",
            ["icon"] = "MissionAssociate.png",
            ["note"] = "Involved in Missions:\
* Bastok Mission 3-3: Jeuno",
            ["type"] = "Mission Associate",
        },
        ["Helga"] = {
            ["_source"] = "npc",
            ["icon"] = "Dialogue.png",
            ["note"] = "Notes:\
- Explains the structure of the Republican military.\
- Identifies herself as first captain of the Gold Musketeers.",
            ["type"] = "Military Information",
        },
        ["Helmut"] = {
            ["_source"] = "npc",
            ["icon"] = "QuestAssociate.png",
            ["note"] = "Involved in Quests:\
* Bait and Switch",
            ["type"] = "Quest Associate",
        },
        ["High Bear"] = {
            ["_source"] = "npc",
            ["icon"] = "QuestGiver.png",
            ["note"] = "Starts Quests:\
* Beadeaux Smog",
            ["type"] = "Quest Giver",
        },
        ["Hildolf"] = {
            ["_source"] = "npc",
            ["icon"] = "QuestGiver.png",
            ["note"] = "Starts Quests:\
* Synergistic Pursuits\
* Synergistic Support\
Description:\
The \"master\" of synergy.",
            ["type"] = "Quest Giver",
        },
        ["Hugues"] = {
            ["_source"] = "npc",
            ["icon"] = "GuildMerchant.png",
            ["type"] = "Synthesis Support",
        },
        ["Hungry Wolf"] = {
            ["_source"] = "npc",
            ["icon"] = "QuestGiver.png",
            ["note"] = "Starts Quests:\
* Smoke on the Mountain\
Involved in Quests:\
* Bait and Switch",
            ["type"] = "Quest Giver",
        },
        ["Indiran"] = {
            ["_source"] = "npc",
            ["icon"] = "Dialogue.png",
            ["type"] = "Citizen",
        },
        ["Invincible Shield"] = {
            ["_source"] = "npc",
            ["icon"] = "BallistaPursuivant.png",
            ["note"] = "Involved in Quests:\
* Lure of the Wildcat (Bastok)\
* Getting a Ballista License\
",
            ["type"] = "Conflict Director",
        },
        ["Iron Eater"] = {
            ["_source"] = "npc",
            ["icon"] = "QuestGiver.png",
            ["note"] = "Starts Quests:\
* The Weight of Your Limits\
Involved in Quests:\
* Fires of Discontent\
* The Doorman\
* Waking the Colossus\
Involved in Missions:\
* Bastok Mission 2-2: Wading Beasts\
* Bastok Mission 3-1: The Four Musketeers\
* Bastok Mission 5-1: Darkness Rising\
* Bastok Mission 6-1: Return of the Talekeeper\
* Bastok Mission 8-1: The Chains that Bind Us\
* Bastok Mission 8-2: Enter the Talekeeper\
* Bastok Mission 9-1: The Salt of the Earth\
* Bastok Mission 9-2: Where Two Paths Converge\
",
            ["type"] = "Mythril Musketeer",
        },
        ["Izabele"] = {
            ["_source"] = "npc",
            ["icon"] = "QuestGiver.png",
            ["note"] = "Starts Quests:\
* The Gustaberg Tour",
            ["type"] = "Quest Giver",
        },
        ["Kachina"] = {
            ["_source"] = "npc",
            ["icon"] = "Dialogue.png",
            ["type"] = "Citizen",
        },
        ["Kaela"] = {
            ["_source"] = "npc",
            ["icon"] = "AdventurersAssistant.png",
            ["note"] = "Involved in Quests:\
* Lure of the Wildcat (Bastok)\
",
            ["type"] = "Adventurer's Assistant",
        },
        ["Kareh'ayollio"] = {
            ["_source"] = "npc",
            ["icon"] = "MissionAssociate.png",
            ["note"] = "Involved in Mission:\
* Heroes' Gambit\
* The Voracious Resurgence\
",
            ["type"] = "Mission Associate",
        },
        ["Karst"] = {
            ["_source"] = "npc",
            ["icon"] = "Bastok.png",
            ["note"] = "Involved in Quests:\
* Waking the Colossus\
Involved in Missions:\
* Bastok Mission 2-3: The Emissary\
* Bastok Mission 5-1: Darkness Rising\
* Bastok Mission 5-2: Xarcabard, Land of Truths\
* Bastok Mission 7-2: On My Way\
* Bastok Mission 9-2: Where Two Paths Converge\
* A Crystalline Prophecy (Mission)\
",
            ["type"] = "President",
        },
        ["Krieger"] = {
            ["_source"] = "npc",
            ["icon"] = "Dialogue.png",
            ["type"] = "Citizen",
        },
        ["Leonhardt"] = {
            ["_source"] = "npc",
            ["icon"] = "QuestAssociate.png",
            ["note"] = "Involved in Quests:\
* Too Many Chefs",
            ["type"] = "Quest Associate",
        },
        ["Lexun-Marixun, W.W."] = {
            ["_source"] = "npc",
            ["icon"] = "ConquestOverseer.png",
            ["note"] = "Notes:\
- Serves Windurstian citizens.\
- Casts Signet and recharges Conquest rings.\
- Accepts crystals and sells Conquest Point rewards.\
- Explains the Conquest system.",
            ["type"] = "Conquest Overseer",
        },
        ["Lorena"] = {
            ["_source"] = "npc",
            ["icon"] = "GuildworkersUnionRepresentative.png",
            ["note"] = "Notes:\
Serves the same function as Macuillie, her counterpart in Northern San d'Oria.",
            ["type"] = "Guild Points",
        },
        ["Lucius"] = {
            ["_source"] = "npc",
            ["icon"] = "QuestNPC.png",
            ["note"] = "Involved in Quests:\
* Riding on the Clouds\
* Waking the Colossus\
* Trust: Bastok - Triggers the cutscene to learn Trust: Volker if you are at least Rank 6.\
Involved in Missions:\
* Bastok Mission 1-1: The Zeruhn Report\
* Bastok Mission 2-3: The Emissary\
* Bastok Mission 3-2: To the Forsaken Mines\
* Bastok Mission 3-3: Jeuno\
* Bastok Mission 5-1: Darkness Rising\
* Bastok Mission 5-2: Xarcabard, Land of Truths\
* Bastok Mission 9-1: The Salt of the Earth\
* Bastok Mission 9-2: Where Two Paths Converge\
",
            ["type"] = "Riding the Clouds",
        },
        ["Lutia"] = {
            ["_source"] = "npc",
            ["icon"] = "Dialogue.png",
            ["note"] = "Notes:\
* Friend to Professor Emanritan.",
            ["type"] = "Scholar",
        },
        ["Malduc"] = {
            ["_source"] = "npc",
            ["icon"] = "MissionGiver.png",
            ["note"] = "Starts Missions:\
* Bastok Missions",
            ["type"] = "Gate Guard",
        },
        ["Manilam"] = {
            ["_source"] = "npc",
            ["icon"] = "QuestAssociate.png",
            ["note"] = "Involved in Quests:\
* Lure of the Wildcat (Bastok)",
            ["type"] = "Quest Associate",
        },
        ["Mathias"] = {
            ["_source"] = "catseye_npc",
            ["icon"] = "QuestNPC.png",
            ["note"] = "Thread Bare:\
* Unlocks harvesting in Crawlers' Nest.\
* Investigate the Vantage Point at Crawlers' Nest J-9, then return.",
            ["type"] = "HELM Unlock Quest",
        },
        ["Matrimonial Coffer"] = {
            ["_source"] = "npc",
            ["icon"] = "SpecialMerchant.png",
            ["type"] = "Standard Merchant",
        },
        ["Mieuseloir"] = {
            ["_source"] = "npc",
            ["icon"] = "QuestAssociate.png",
            ["note"] = "Involved in Quest:\
* The Return of Mieuseloir\
* Mieuseloir's Marvelous Adventure\
",
            ["type"] = "Quest Associate",
        },
        ["Mighty Fist"] = {
            ["_source"] = "npc",
            ["icon"] = "QuestGiver.png",
            ["note"] = "Starts Quests:\
* The Darksmith\
Involved in Quests:\
* Dark Legacy",
            ["type"] = "Quest Giver",
        },
        ["Mighty Hammer"] = {
            ["_source"] = "catseye_npc",
            ["icon"] = "DailyQuestGiver.png",
            ["link"] = "https://www.bg-wiki.com/ffxi/CatsEyeXI_Systems/Quests",
            ["note"] = "Lode of Gil:\
- Starts the Lode of Gil quest.\
- Pays for Darksteel Ore.",
            ["type"] = "Darksteel Ore Buyer",
        },
        ["Mih Ketto"] = {
            ["_source"] = "npc",
            ["icon"] = "Dialogue.png",
            ["type"] = "Citizen",
        },
        ["Militant Gale"] = {
            ["_source"] = "npc",
            ["icon"] = "Cutscene.png",
            ["note"] = "Involved in Quests:\
* Bait and Switch\
",
            ["type"] = "Cutscene NPC",
        },
        ["Moyoyo"] = {
            ["_source"] = "npc",
            ["icon"] = "Dialogue.png",
            ["type"] = "Citizen",
        },
        ["Mukunda"] = {
            ["_source"] = "npc",
            ["icon"] = "Dialogue.png",
            ["type"] = "Citizen",
        },
        ["Mythily"] = {
            ["_source"] = "npc",
            ["icon"] = "ImmigrationNPC.png",
            ["note"] = "Notes:\
- Changes a character's allegiance to Bastok.\
- Fee depends on Bastok's current Conquest rank.\
- Cannot change allegiance while a mission is active.\
- Previous national rank, Conquest Points, and outpost progress are retained for when you return.",
            ["type"] = "Immigration",
        },
        ["Nabima-Hyonbima"] = {
            ["_source"] = "npc",
            ["icon"] = "Dialogue.png",
            ["type"] = "Citizen",
        },
        ["Naji"] = {
            ["_source"] = "npc",
            ["icon"] = "Guard.png",
            ["note"] = "Involved in Quests:\
* Riding on the Clouds\
* The Doorman\
* Trust: Bastok\
* Adventurer Appreciation Campaign 2010\
Involved in Missions:\
* Bastok Mission 1-1: The Zeruhn Report\
* Bastok Mission 2-3: The Emissary\
* Bastok Mission 3-1: The Four Musketeers\
* Bastok Mission 5-1: Darkness Rising\
* Bastok Mission 6-2: The Pirates' Cove\
* Bastok Mission 9-2: Where Two Paths Converge\
* Promathia Mission 3-1: Call of the Wyrmking\
* Aht Urhgan Mission 38: Stirrings of War\
* Aht Urhgan Mission 39: Allied Rumblings\
* Aht Urhgan Mission 40: Unraveling Reason\
* Aht Urhgan Mission 41: Light of Judgment\
* A Crystalline Prophecy (Mission)\
* The Voracious Resurgence #2: The Brygid Cup\
",
            ["type"] = "Mythril Musketeer",
        },
        ["Nogga"] = {
            ["_source"] = "npc",
            ["icon"] = "Merchant.png",
            ["type"] = "Standard Merchant",
        },
        ["Olaf"] = {
            ["_source"] = "npc",
            ["icon"] = "Merchant.png",
            ["type"] = "Standard Merchant",
        },
        ["Patricius"] = {
            ["_source"] = "npc",
            ["icon"] = "AdventurersAssistant.png",
            ["note"] = "Notes:\
Provides information about linkshells:\
- Explains linkshells, linkpearls, and pearlsacks.\
- Explains how to create, equip, and manage them.",
            ["type"] = "Adventurer's Assistant",
        },
        ["Patt-Pott"] = {
            ["_source"] = "npc",
            ["icon"] = "windurst.png",
            ["note"] = "Involved in Missions:\
* Windurst Mission 2-3: The Three Kingdoms",
            ["type"] = "Consulate Representative",
        },
        ["Pensive Beast"] = {
            ["_source"] = "npc",
            ["icon"] = "Cutscene.png",
            ["note"] = "Involved in Quests:\
* Bait and Switch",
            ["type"] = "Cutscene NPC",
        },
        ["Pius"] = {
            ["_source"] = "npc",
            ["icon"] = "MissionAssociate.png",
            ["note"] = "Involved in Missions:\
* San d'Oria Mission 2-3: Journey Abroad\
* Windurst Mission 2-3: The Three Kingdoms",
            ["type"] = "Mission Associate",
        },
        ["Quasim"] = {
            ["_source"] = "npc",
            ["icon"] = "AdventurersAssistant.png",
            ["note"] = "Notes:\
* This NPC will display a map of Bastok.",
            ["type"] = "Adventurer's Assistant",
        },
        ["Raibaht"] = {
            ["_source"] = "npc",
            ["icon"] = "QuestGiver.png",
            ["note"] = "Starts Quests:\
* Hyper Active\
* Teak Me to the Stars\
* The Naming Game\
* Dark Legacy\
Involved in Quests:\
* Lure of the Wildcat (Bastok)\
* The Usual\
* Riding on the Clouds\
* Vegetable Vegetable Crisis\
* Vegetable Vegetable Frustration\
Involved in Missions:\
* Bastok Mission 1-2: A Geological Survey\
* Bastok Mission 7-1: The Final Image\
",
            ["type"] = "Riding the Clouds",
        },
        ["Remus"] = {
            ["_source"] = "npc",
            ["icon"] = "Dialogue.png",
            ["type"] = "Citizen",
        },
        ["Riault"] = {
            ["_source"] = "npc",
            ["icon"] = "Dialogue.png",
            ["type"] = "Citizen",
        },
        ["Romero"] = {
            ["_source"] = "npc",
            ["icon"] = "GuildMerchant.png",
            ["type"] = "Synthesis Support",
        },
        ["Romualdo"] = {
            ["_source"] = "npc",
            ["icon"] = "QuestGiver.png",
            ["note"] = "Starts Quests:\
* Faded Promises\
* Message on the Wind\
Involved in Quests:\
* Stamp Hunt",
            ["type"] = "Quest Giver",
        },
        ["Salim"] = {
            ["_source"] = "npc",
            ["icon"] = "QuestGiver.png",
            ["note"] = "Starts Quests:\
* Bait and Switch",
            ["type"] = "Quest Giver",
        },
        ["Savae E Paleade"] = {
            ["_source"] = "npc",
            ["icon"] = "SandOria.png",
            ["note"] = "Involved in Missions:\
* Journey Abroad",
            ["type"] = "Consulate Representative",
        },
        ["Striking Snake"] = {
            ["_source"] = "npc",
            ["icon"] = "QuestAssociate.png",
            ["note"] = "Involved in Quests:\
* Bait and Switch\
* The Long March North",
            ["type"] = "Quest Associate",
        },
        ["Takiyah"] = {
            ["_source"] = "npc",
            ["icon"] = "RegionalVendor.png",
            ["type"] = "Regional Vendor",
        },
        ["Taulluque"] = {
            ["_source"] = "npc",
            ["icon"] = "PastEventWatcher.png",
            ["note"] = "Notes:\
* Replays cutscenes for this area.",
            ["type"] = "Past Event Watcher",
        },
        ["Tomasa"] = {
            ["_source"] = "npc",
            ["icon"] = "Merchant.png",
            ["type"] = "Standard Merchant",
        },
        ["Topuru-Kuperu"] = {
            ["_source"] = "npc",
            ["icon"] = "Dialogue.png",
            ["type"] = "Citizen",
        },
        ["Udine A.M.A.N."] = {
            ["_source"] = "npc",
            ["icon"] = "Dialogue.png",
            ["type"] = "Citizen",
        },
        ["Umon-Paumon"] = {
            ["_source"] = "npc",
            ["icon"] = "QuestAssociate.png",
            ["note"] = "Involved in Quest:\
* The Return of Mieuseloir\
* Mieuseloir's Marvelous Adventure\
",
            ["type"] = "Quest Associate",
        },
        ["Unlucky Rat"] = {
            ["_source"] = "npc",
            ["icon"] = "QuestGiver.png",
            ["note"] = "Starts Quests:\
* Mean Machine",
            ["type"] = "Scroll Quest",
        },
        ["Veit"] = {
            ["_source"] = "npc",
            ["icon"] = "Dialogue.png",
            ["type"] = "Citizen",
        },
        ["Vicious Eye"] = {
            ["_source"] = "npc",
            ["icon"] = "GuildMerchant.png",
            ["note"] = "Notes:\
- Always open.\
- Has independent stock from Amulya.\
- Sells the same selection as Lucretia and Mololo.",
            ["type"] = "Smithing Merchant",
        },
        ["Viresefilant"] = {
            ["_source"] = "npc",
            ["icon"] = "Dialogue.png",
            ["type"] = "Citizen",
        },
        ["Vladinek"] = {
            ["_source"] = "npc",
            ["icon"] = "QuestAssociate.png",
            ["note"] = "Involved in Quests:\
* The Search for Goldmane",
            ["type"] = "Quest Associate",
        },
        ["Wise Owl"] = {
            ["_source"] = "npc",
            ["icon"] = "GuildMerchant.png",
            ["note"] = "Notes:\
His galkan name is Garbenn.",
            ["type"] = "Synthesis Support",
        },
        ["Yestin-Ovestin"] = {
            ["_source"] = "catseye_npc",
            ["icon"] = "QuestAssociate.png",
            ["link"] = "https://www.bg-wiki.com/ffxi/CatsEyeXI_Systems/Quests#Forged_in_Frost_and_Flame",
            ["note"] = "Involved in Quest:\
- Forged in Frost and Flame\
- Helps obtain the Beorc Sword for Rune Fencer.",
            ["type"] = "RUN Artifact Quest Associate",
        },
        ["Zenobia"] = {
            ["_source"] = "npc",
            ["icon"] = "Jeuno.png",
            ["type"] = "Consulate Representative",
        },
        ["Matabe"] = {
            ["_source"] = "npc",
            ["icon"] = "QuestAssociate.png",
            ["note"] = "Involved in Quests:\
* Faded Promises",
            ["type"] = "Quest Associate",
        },
    },
    ["objects"] = {
        ["Celebratory Chest"] = {
            ["_source"] = "item",
            ["icon"] = "TreasureCasket.png",
            ["note"] = "A stylized national festive coffer manifest within prominent sovereign audience chambers. Opening this repository distributes unique holiday cosmetics or registers custom server event milestones.",
            ["type"] = "Loot Container",
        },
        ["Door:Aide's Office"] = {
            ["_source"] = "item",
            ["icon"] = "Door.png",
        },
        ["Door:Audience Chamber"] = {
            ["_source"] = "item",
            ["icon"] = "Door.png",
        },
        ["Door:Blacksmiths' Guild"] = {
            ["_source"] = "item",
            ["icon"] = "Door.png",
        },
        ["Door:Cannonry"] = {
            ["_source"] = "item",
            ["icon"] = "Door.png",
        },
        ["Door:Cermet Refinery"] = {
            ["_source"] = "item",
            ["icon"] = "Door.png",
        },
        ["Door:Cid's Lab"] = {
            ["_source"] = "item",
            ["icon"] = "Door.png",
        },
        ["Door:Conference Room"] = {
            ["_source"] = "item",
            ["icon"] = "Door.png",
        },
        ["Door:Cornelia's Room"] = {
            ["_source"] = "item",
            ["icon"] = "Door.png",
        },
        ["Door:Craftsmen's Eatery"] = {
            ["_source"] = "item",
            ["icon"] = "Door.png",
        },
        ["Door:Darksteel Forge"] = {
            ["_source"] = "item",
            ["icon"] = "Door.png",
        },
        ["Door:Dept. of Industry"] = {
            ["_source"] = "item",
            ["icon"] = "Door.png",
        },
        ["Door:Gunpowder Room"] = {
            ["_source"] = "item",
            ["icon"] = "Door.png",
        },
        ["Door:Jeunoan Consul"] = {
            ["_source"] = "item",
            ["icon"] = "Door.png",
        },
        ["Door:President's Office"] = {
            ["_source"] = "item",
            ["icon"] = "Door.png",
        },
        ["Door:Presidential Suite"] = {
            ["_source"] = "item",
            ["icon"] = "Door.png",
        },
        ["Door:San d'Orian Consul"] = {
            ["_source"] = "item",
            ["icon"] = "Door.png",
        },
        ["Door:Windurstian Consul"] = {
            ["_source"] = "item",
            ["icon"] = "Door.png",
        },
        ["Large Switch"] = {
            ["_source"] = "item",
            ["icon"] = "Switch.png",
            ["note"] = "A polished mechanical lever built into the laboratory walls. Throwing the heavy toggle handles manufacturing subroutines and advances high-tier Republic mission campaigns.",
            ["type"] = "Dungeon Switch",
        },
        ["Matrimonial Coffer"] = {
            ["_source"] = "item",
            ["icon"] = "TreasureCasket.png",
            ["note"] = "An ornate, locked ceremonial chest placed within prominent national audience chambers. Opening the coffer uncovers long-lost historical logs or reviews festive holiday scenarios.",
            ["type"] = "Loot Container",
        },
        ["Mawl'gofaur"] = {
            ["_source"] = "item",
            ["icon"] = "QuestNode.png",
            ["note"] = "A distinct natural ground landmark or unique ancient monument. Examining it updates advanced storyline milestones or validates your tracking journals across high-tier content.",
            ["type"] = "Quest Node",
        },
        ["Medium Switch"] = {
            ["_source"] = "item",
            ["icon"] = "Switch.png",
            ["note"] = "A weighted lever apparatus set directly into the laboratory framing. Toggling the handle shifts manufacturing circuits to advance high-ranking Republic storyline events.",
            ["type"] = "Dungeon Switch",
        },
        ["Small Switch"] = {
            ["_source"] = "item",
            ["icon"] = "Switch.png",
            ["note"] = "A compact mechanical lever mounted to the laboratory wall circuits. Toggling the handle adjusts manufacturing parameters to advance active Republic rank mission milestones.",
            ["type"] = "Dungeon Switch",
        },
    },
}
