...
local statesByEntity = {
    ['Self'] = T{ 'World', 'Tactical', 'Log Off', 'Resting', 'Fishing', 'Crafting', 'Gathering' },
...
GetEditWidgetsFor = function(entity, stateName)
    ...
    if (entityName == 'Self') then
        ...
        if (plateName == 'Log Off') then
            return selfLogOffWidgets;
        end
...
function GetWidgetDefaults(widget)
    ...
    if (widget == 'Logout (module)') then return { enabled = true }; end
...
function GetChecklistActiveSettings(widget)
    ...
    if (widget == 'Logout (module)') then
        local global = state.GetGlobalSettings(globalDefaults);
        global.logout = global.logout or {};
        if (global.logout.enabled == nil) then global.logout.enabled = true; end
...
function EnsureSelectedWidgetAllowed()
    ...
    if (entityName == 'Self' and stateName == 'Resting' and selectedWidget == 'Logout (module)') then
        selectedWidget = 'Resting (module)';
    end

    if (entityName == 'Self' and stateName == 'Log Off' and selectedWidget == 'Resting (module)') then
        selectedWidget = 'Logout (module)';
    end
...
function GetPreviewSelection()
    if (selectedTab == 'Modules') then
        ...
        elseif (selectedModuleName == 'Resting') then
            widgetKey = 'Resting';
        elseif (selectedModuleName == 'Logout') then
            widgetKey = 'Logout';
        ...
        if (selectedModuleName == 'Resting' or selectedModuleName == 'Logout' or selectedModuleName == 'Fishing' or selectedModuleName == 'Crafting') then
            entityName = 'Self';
            stateName = 'Idle';
        end
    ...
    if (selectedTab == 'Plates') then
        ...
        elseif (selectedWidget == 'Logout (module)') then
            context = {
                entityName = 'Self',
                stateName = 'Idle',
                widgetKey = 'Logout',
...
function LibraPlatesSettingsDrawLogoutModuleSettings(settings, hideActive)
    settings.logout = settings.logout or {};

    if (settings.logout.enabled == nil) then settings.logout.enabled = true; end
    if (settings.logout.displayMode == nil) then settings.logout.displayMode = 'Bar'; end
    if (settings.logout.width == nil) then settings.logout.width = 180; end
    if (settings.logout.height == nil) then settings.logout.height = 12; end
    if (settings.logout.ringSize == nil) then settings.logout.ringSize = 88; end
    if (settings.logout.ringThickness == nil) then settings.logout.ringThickness = 10; end
    if (settings.logout.offsetX == nil) then settings.logout.offsetX = 0; end
    if (settings.logout.offsetY == nil) then settings.logout.offsetY = 38; end
    if (settings.logout.color == nil) then settings.logout.color = { 0.55, 0.95, 0.35, 1.0 }; end
    if (settings.logout.backgroundColor == nil) then settings.logout.backgroundColor = { 0.10, 0.10, 0.10, 1.0 }; end
    if (settings.logout.borderColor == nil) then settings.logout.borderColor = { 1.0, 1.0, 1.0, 1.0 }; end
    if (settings.logout.borderSize == nil) then settings.logout.borderSize = 0; end
    if (settings.logout.fontSize == nil) then settings.logout.fontSize = 12; end
    if (settings.logout.textColor == nil) then settings.logout.textColor = { 0.0, 0.0, 0.0, 1.0 }; end
    if (settings.logout.textOutlineColor == nil) then settings.logout.textOutlineColor = { 1.0, 1.0, 1.0, 1.0 }; end
    if (settings.logout.textOutlineSize == nil) then settings.logout.textOutlineSize = 1; end

    if (hideActive ~= true) then
        DrawCheckbox('Active', settings.logout.enabled == true, function(value)
            settings.logout.enabled = value == true;
            state.Save();
        end);
    end

    if (settings.logout.enabled ~= true and hideActive ~= true) then
        return;
    end

    DrawInlineComboRow('Display', T{ 'Bar', 'Ring' }, settings.logout.displayMode or 'Bar', function(value)
        settings.logout.displayMode = value;
        state.Save();
    end, 'LogoutDisplayMode');

    if (tostring(settings.logout.displayMode or 'Bar') == 'Ring') then
        local ringSize, ringSizeChanged, ringThickness, ringThicknessChanged = DrawPlacementPair('Ring size', settings.logout.ringSize, 'LogoutRingSize', 'Ring thickness', settings.logout.ringThickness, 'LogoutRingThickness', 1, 300, 1);
        if (ringSizeChanged == true or ringThicknessChanged == true) then
            settings.logout.ringSize = ringSize;
            settings.logout.ringThickness = ringThickness;
            state.Save();
        end
    else
        local width, widthChanged, height, heightChanged = DrawPlacementPair('Width', settings.logout.width, 'LogoutWidth', 'Height', settings.logout.height, 'LogoutHeight', 1, 900, 1);
        if (widthChanged == true or heightChanged == true) then
            settings.logout.width = width;
            settings.logout.height = height;
            state.Save();
        end
    end

    local x, xChanged, y, yChanged = DrawPlacementPair('Position X', settings.logout.offsetX, 'LogoutX', 'Position Y', settings.logout.offsetY, 'LogoutY', -500, 500, 1);
    if (xChanged == true or yChanged == true) then
        settings.logout.offsetX = x;
        settings.logout.offsetY = y;
        state.Save();
    end

    local fillColor, fillChanged = DrawSettingsColor('Fill color', settings.logout.color, 'LogoutFillColor');
    if (fillChanged == true) then
        settings.logout.color = fillColor;
        state.Save();
    end

    local backgroundColor, backgroundChanged = DrawSettingsColor('Background color', settings.logout.backgroundColor, 'LogoutBackgroundColor');
    if (backgroundChanged == true) then
        settings.logout.backgroundColor = backgroundColor;
        state.Save();
    end

    local borderColor, borderChanged, borderSize, borderSizeChanged = DrawColorAndPlacementRow(
        'Border color',
        settings.logout.borderColor,
        'LogoutBorderColor',
        'Border size',
        settings.logout.borderSize,
        'LogoutBorderSize',
        0,
        24,
        1
    ...
    if (borderChanged == true or borderSizeChanged == true) then
        settings.logout.borderColor = borderColor;
        settings.logout.borderSize = borderSize;
        state.Save();
    end

    local fontSize, fontSizeChanged, textColor, textColorChanged = DrawPlacementAndColorRow(
        'Text size',
        settings.logout.fontSize,
        'LogoutFontSize',
        6,
        48,
        1,
        'Text color',
        settings.logout.textColor,
        'LogoutTextColor'
    );
    if (fontSizeChanged == true or textColorChanged == true) then
        settings.logout.fontSize = fontSize;
        settings.logout.textColor = textColor;
        state.Save();
    end

    local outlineSize, outlineSizeChanged, outlineColor, outlineColorChanged = DrawPlacementAndColorRow(
        'Outline size',
        settings.logout.textOutlineSize,
        'LogoutOutlineSize',
        0,
        12,
        1,
        'Outline color',
        settings.logout.textOutlineColor,
        'LogoutOutlineColor'
    );
    if (outlineSizeChanged == true or outlineColorChanged == true) then
        settings.logout.textOutlineSize = outlineSize;
        settings.logout.textOutlineColor = outlineColor;
        settings.logout.textOutlineEnabled = outlineSize > 0;
        state.Save();
    end
end
...
function LibraPlatesSettingsToStorageStateName(value)
...
function DrawSelectedEditorGeneral()
...
function DrawSelectedEditorTargeting()
...
function DrawSelectedEditorModules()
if (selectedTab == 'Modules') then
        local selectedModuleName = tostring(selectedModuleWidget or ''):gsub('%s+', ' '):gsub('^%s+', ''):gsub('%s+$', '');
        ...
        if (selectedModuleName == 'Logout') then
            LibraPlatesSettingsDrawLogoutModuleSettings(state.GetGlobalSettings(globalDefaults));
            return;
        end
...
function DrawSelectedEditorPlatesTargetWidgets()
    if (
...
function DrawSelectedEditorPlatesModules()
    ...
    if (selectedWidget == 'Logout (module)') then
        LibraPlatesSettingsDrawLogoutModuleSettings(state.GetGlobalSettings(globalDefaults), true);
        return;
    end
...
function DrawSelectedEditorPlatesName()
...
function DrawSelectedEditorPlatesWidgetWithStorageDefaults(widgetName, defaults, drawFn, extras)
...
function DrawSelectedEditorPlatesPetDefaultsWidget(widgetName, defaults, resourceName)
    if (selectedWidget ~= widgetName) then
        return;
    end
...
function DrawSelectedEditorPlates()
    if (selectedTab ~= 'Plates') then
    ...
    if (selectedWidget == 'Name' or selectedWidget == 'Background' or selectedWidget == 'Job' or selectedWidget == 'Level' or selectedWidget == 'ID' or selectedWidget == 'Distance' or selectedWidget == 'Type line' or selectedWidget == 'Buffs' or selectedWidget == 'Debuffs' or selectedWidget == 'Game mode icon' or selectedWidget == 'Bazaar icon' or selectedWidget == 'Linkshell icon' or selectedWidget == 'Away icon' or selectedWidget == 'Disconnect icon' or selectedWidget == 'Anon icon' or selectedWidget == 'Follow icon' or selectedWidget == 'Party leader icon' or selectedWidget == 'Alliance leader icon' or selectedWidget == 'Stars icon' or selectedWidget == 'Level sync icon' or selectedWidget == 'New adventurer icon' or selectedWidget == 'Icon' or selectedWidget == 'NPC icon' or selectedWidget == 'Object icon' or selectedWidget == 'HP Bar' or selectedWidget == 'MP Bar' or selectedWidget == 'TP Bar' or selectedWidget == 'Cast bar' or selectedWidget == 'Pet timer' or selectedWidget == 'Pet state' or selectedWidget == 'Ward timer' or selectedWidget == 'Rage timer' or selectedWidget == 'Sic' or selectedWidget == 'Ready bar' or selectedWidget == 'Reward' or selectedWidget == 'Maneuvers' or selectedWidget == 'Target' or selectedWidget == 'Subtarget' or selectedWidget == 'Target (module)' or selectedWidget == 'Subtarget (module)' or selectedWidget == 'Peer (module)' or selectedWidget == 'Enmity (module)' or selectedWidget == 'Resting (module)' or selectedWidget == 'Logout (module)' or selectedWidget == 'Crafting (module)' or selectedWidget == 'Fishing (module)' or selectedWidget == 'Quick Menu (module)') then
        return;
    end
