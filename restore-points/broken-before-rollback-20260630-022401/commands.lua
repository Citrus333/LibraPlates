...
function commands.Handle(e)
    ...
    if (subcommand == 'blmodel' or subcommand == 'blacklistmodel') then
        ...
        if (action == 'on' or action == 'enable') then
            ...
            state.Save();
            log.Warn('Blacklist Fomor appearance setting enabled, but runtime body replacement is disabled to prevent invisible players.');
            return;
        end
        ...
        if (action == 'off' or action == 'disable') then
            ...
            state.Save();
            log.Info('Blacklist Fomor appearance disabled.');
            return;
        end
        ...
        log.Info(
            'Blacklist Fomor appearance=' .. tostring(settings.modelReplaceUseFomor ~= false) ..
            ' rename=' .. tostring(settings.replaceName ~= false) ..
            ' race=' .. tostring(settings.modelReplaceRace) ..
